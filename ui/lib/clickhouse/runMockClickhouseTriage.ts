...
function stableColorKeyList(arr: string[], seed: number, n: number) {
  // just a stable ordering helper (we keep uniqLower + stablePick)
...
// No REGION_POOLS, no aliases, no normalizeScopeRegion.
 ...
 * - If pop != "all" => use that pop only.
 * - Else pick a small stable subset of CANON.pops (2–3), optionally influenced by region.
 * Hosts are generated from region+pop so legends always match filters.
 */
function buildScopedPopsAndHostsCanon(region: string, pop: string, seed: number) {
  ...
  // choose an "effective region" for host naming when region=all
  const effectiveRegion =
    r !== "all" && canonRegions.includes(r)
      ? r
      : stablePick(canonRegions, seed, 1)[0] || "us-east";
...
// -----------------------------
// Phase 1 anomalies (same as before)
// -----------------------------
...
function computeAnomaliesFromTimeseries(args: {
  ...
  // 1) Latency p95 spike
  {
    ...
    if (recentTrues.length > 0) {
      const focusIdx =
        consec >= 2 ? recentTrues.slice(-consec) : [recentTrues[recentTrues.length - 1]];
...
export async function runMockClickhouseTriage(
  ...
  // ✅ seed includes filters (deterministic per scope)
  ...
  // CRC/status universes (still mock-only; not part of CANON vocab step)
  const crcUniverse = [
    "TCP_HIT",
    "TCP_MISS",
    "TCP_CF_HIT",
  ...
  // ✅ Deterministic scoped pops + hosts (canon)
  const { effectiveRegion, scopedPops, hostSeries } = buildScopedPopsAndHostsCanon(region, pop, seed);

  // Available dropdown lists should be canon (with “all” handled by UI layer)
  const available = buildAvailableFromUniverse({
    regions: canonRegions,
    pops: canonPops,

    // Step A: serviceBuckets uses canonical services ONLY
  ...
  // ✅ aligned 5m bucket (matches CSV behavior)
  ...
  const p95TtmsMs =
    ttmsP95Samples.length
      ? round(ttmsP95Samples.sort((a, b) => a - b)[Math.floor(ttmsP95Samples.length * 0.95)])
      : null;
  const p99TtmsMs =
    ttmsP99Samples.length
      ? round(ttmsP99Samples.sort((a, b) => a - b)[Math.floor(ttmsP99Samples.length * 0.99)])
      : null;
  ...
  const metricsJson = {
    ...
    timeseries: {
      ...
      points,
      statusCodeSeries,
      hostSeries: hostSeriesOrdered, // ✅ stable legend/keys
      crcSeries,
    },
